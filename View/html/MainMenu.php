 <!-- ########## START: LEFT PANEL ########## -->
 <div class="br-logo"><a href="../UsuHome/index.php"><span>[</span>Certificados<span>]</span></a></div>
 <div class="br-sideleft overflow-y-auto">
   <div>
 <label class="sidebar-label pd-x-10 mg-t-20">Menu - 
    <?php if($_SESSION["rol_id"]=='1'){?>
     USUARIO 
    <?php }else{?>
       ADMINISTRADOR 
      <?php }?>
   </label>
   </div>
   <div class="br-sideleft-menu">

     <!-- AHORA REALIZAMO UN IF PARA VER SI ES ADMINISTRADOR O USUARIOS DEACUERDO A 
      ELLO SE MOSTRARAN LAS VENTANAS YA SES DE ADMIN O USUARIO -->

     <!-- AHORA REALIZAMO UN IF PARA VER SI ES ADMINISTRADOR O USUARIOS DEACUERDO A 
      ELLO SE MOSTRARAN LAS VENTANAS YA SES DE ADMIN O USUARIO -->

     <a href="../UsuHome/index.php" class="br-menu-link">
       <div class="br-menu-item">
         <i class="menu-item-icon fa fa-home tx-20"></i>
         <span class="menu-item-label">Home</span>
       </div><!-- menu-item -->
     </a><!-- br-menu-link -->

     <?php
      if ($_SESSION["rol_id"]=='1' ) {
      ?>
       <a href="../UsuCurso/index.php" class="br-menu-link">
         <div class="br-menu-item">
           <i class="menu-item-icon fa fa-book tx-20"></i>
           <span class="menu-item-label">Mis Curso</span>
         </div><!-- menu-item -->
       </a><!-- br-menu-link -->


     <?php
      } else {
      ?>

       <a href="../AdminMntUsuario/index.php" class="br-menu-link">
         <div class="br-menu-item">
           <i class="menu-item-icon fa fa-user tx-20"></i>
           <span class="menu-item-label">Mnt Usuario</span>
         </div><!-- menu-item -->
       </a><!-- br-menu-link -->

       <a href="../AdminMntCurso/index.php" class="br-menu-link">
         <div class="br-menu-item">
           <i class="menu-item-icon fa fa-book tx-20"></i>
           <span class="menu-item-label">Mnt Curso</span>
         </div><!-- menu-item -->
       </a><!-- br-menu-link -->
       <a href="../AdminMntInstructor/index.php" class="br-menu-link">
         <div class="br-menu-item">
           <i class="menu-item-icon fa fa-users tx-20"></i>
           <span class="menu-item-label">Mnt Instructor</span>  
         </div><!-- menu-item -->
       </a><!-- br-menu-link -->

       <a href="../AdminMntCategoria/index.php" class="br-menu-link">
         <div class="br-menu-item">
           <i class="menu-item-icon fa fa-server tx-20"></i>
           <span class="menu-item-label">Mnt Categroria</span>
         </div><!-- menu-item -->
       </a><!-- br-menu-link -->

       <a href="../AdminMntDetalleCertificado/index.php" class="br-menu-link">
         <div class="br-menu-item">
           <i class="menu-item-icon fa fa-clone tx-20"></i>
           <span class="menu-item-label">Mnt Detalle Certificado</span>
         </div><!-- menu-item -->
       </a><!-- br-menu-link -->
       
       <?php
      }
      ?>
     <a href="../UsuPerfil/" class="br-menu-link">
       <div class="br-menu-item">
         <i class="menu-item-icon fa fa-gear tx-20"></i>
         <span class="menu-item-label">Perfil</span>
       </div><!-- menu-item -->
     </a><!-- br-menu-link -->

     <a href="../html/Logout.php" class="br-menu-link">
       <div class="br-menu-item">
         <i class=" icon icon ion-power tx-20"></i>
         <span class="menu-item-label">Cerrar Session</span>
       </div><!-- menu-item -->
     </a><!-- br-menu-link -->

   </div><!-- br-sideleft-menu -->

   <br>
 </div><!-- br-sideleft -->